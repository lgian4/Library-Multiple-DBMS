...
namespace Library_App.MsSQLServerExpress
{
    class AccountMethods : IAccountMethodStrategy
    {
        ...
        public void Add(Account account)
        {
            ...
            parameters.Enqueue(new SqlParameter("@FullName", account.FullName));
            parameters.Enqueue(new SqlParameter("@Address", account.Address));
            parameters.Enqueue(new SqlParameter("@PhoneNumber", account.PhoneNumber));

            // have to explicitly set to date type
            SqlParameter publishedParameter = new SqlParameter("@Opened", account.Opened);
            parameters.Enqueue(publishedParameter);
            parameters.Enqueue(new SqlParameter("@Level", account.AccountLevel));
            parameters.Enqueue(new SqlParameter("@Username", account.Username));

            string salt = AuthHelper.CreateSalt(32);
            string password = AuthHelper.CreatePasswordHash(account.Password, salt);
            parameters.Enqueue(new SqlParameter("@Password", password));
            parameters.Enqueue(new SqlParameter("@Salt", salt));
            #endregion
        ...
        public void ChangePassword(string username, string oldPassword, string newPassword)
        {
            ...
            Queue<object> parameters = new Queue<object>();
            parameters.Enqueue(new SqlParameter("@Username", username));
            string salt = AuthHelper.CreateSalt(32);
            string password = AuthHelper.CreatePasswordHash(newPassword, salt);
            parameters.Enqueue(new SqlParameter("@Password", password));
            parameters.Enqueue(new SqlParameter("@Salt", salt));
            parameters.Enqueue(new SqlParameter("@ID", id));
        ...
        public List<Account> Find(string inputquery)
        {
            ...
            Queue<object> parameters = new Queue<object>();
            parameters.Enqueue(new SqlParameter("@inputQuery", inputquery.Trim()));
        ...
        public void Remove(int id)
        {
            ...
            string query = "DELETE  FROM tblAccount WHERE(((tblAccount.ID) = @ID)); ";
            DBHelper.ExecuteQuery(query, parameters);
        }
        ...
        public void Update(Account account)
        {
            ...
            parameters.Enqueue(new SqlParameter("@ID", account.ID));
