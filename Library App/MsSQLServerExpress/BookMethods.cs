...
{
    /// <summary>
    /// Book Methods tha use MS Access Database
    /// </summary>
    public class BookMethods : IBookMethodStrategy
    {
        ...
        public void Add(Book book)
        {
            ...
            parameters.Enqueue(new SqlParameter("@Title", book.Title));
            parameters.Enqueue(new SqlParameter("@Author", book.Author));
            parameters.Enqueue(new SqlParameter("@Publisher", book.Publisher));

            // have to explicitly set to date type
            SqlParameter publishedParameter = new SqlParameter("@PublishedDate", book.PublishedDate);
            publishedParameter.DbType = System.Data.DbType.DateTime;
        ...
        public void Remove(int id)
        {
            ...
            string query = "DELETE FROM tblBook WHERE(((tblBook.ID) = @ID)); ";
            DBHelper.ExecuteQuery(query, parameters);
        }
        /// <summary>
        ...
        public void Update(Book book)
        {
            ...
            SqlParameter publishedParameter = new SqlParameter("@PublishedDate", book.PublishedDate);
            parameters.Enqueue(publishedParameter);
            parameters.Enqueue(new SqlParameter("@ID", book.ID));
