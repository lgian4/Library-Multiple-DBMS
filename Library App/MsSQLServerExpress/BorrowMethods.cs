...
namespace Library_App.MsSQLServerExpress
{
    class BorrowMethods : IBorrowMethodStrategy
    {
        ...
        public List<JoinBorrow> Find(string text)
        {
            ...
            Queue<object> parameters = new Queue<object>();
            parameters.Enqueue(new SqlParameter("@Query", text));
            DataSet result = DBHelper.ExecuteReturnedQuery(query, parameters);
            List<JoinBorrow> resultAccount = new List<JoinBorrow>();
            DateTime testDate, testdate2, testdate3, testdate4;
            foreach (DataRow dataRow in result.Tables[0].Rows)
            {
        ...
        public void Borrow(Borrow borrow)
        {
            ...
            Queue<object> parameters = new Queue<object>();
            parameters.Enqueue(new SqlParameter("@BookId", borrow.BookID));
            parameters.Enqueue(new SqlParameter("@AccountId", borrow.AccountID));

            // have to explicitly set to date type
            SqlParameter publishedParameter1 = new SqlParameter("@Borrowed", borrow.Borrowed);
            publishedParameter1.DbType = DbType.Date;
            parameters.Enqueue(publishedParameter1);
            string query = "INSERT INTO tblBorrow ( BookId, AccountId, Borrowed ) VALUES (@BookId, @AccountId, @Borrowedrned)";
        ...
        public List<JoinBorrow> AccountHistory(int accountId)
        {
            ...
            Queue<object> parameters = new Queue<object>();
            parameters.Enqueue(new SqlParameter("@AccountID", accountId));
            DataSet result = DBHelper.ExecuteReturnedQuery(query, parameters);
            List<JoinBorrow> resultAccount = new List<JoinBorrow>();
            foreach (DataRow dataRow in result.Tables[0].Rows)
            {
        ...
        private void Return(Borrow borrow)
        {
            ...
            Queue<object> parameters = new Queue<object>();
            // have to explicitly set to date type
            SqlParameter publishedParameter = new SqlParameter("@Returned", borrow.Returned);
            parameters.Enqueue(publishedParameter);

            publishedParameter = new SqlParameter("@Fine", borrow.Fine);
            publishedParameter.DbType = DbType.Currency;
            parameters.Enqueue(publishedParameter);

            parameters.Enqueue(new SqlParameter("@ID", borrow.ID));
